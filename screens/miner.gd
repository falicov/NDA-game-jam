...
func _process(delta: float) -> void:
	...
	elif(_current_tile != _mining_tile && _current_tile != map.mine_base || _current_tile == Vector2i(0,0)):
	...
	#mine a tile
	if(map.local_to_map(position) == _mining_tile && _current_tile == _mining_tile):
		print("I'm mining!")
		#if this doesn't work... there's some problem with the tiles
		var tile_data:TileData = map.get_cell_tile_data(_mining_tile)
		...
		|| (_time_mined >= _time_mined*3/5 && anim_stage == 2)):
			var source_id = map.get_cell_source_id(_mining_tile)
			var atlas_coords = map.get_cell_atlas_coords(_mining_tile)
			map.set_cell(_mining_tile, source_id, Vector2i(atlas_coords.x, atlas_coords.y+1))
			anim_stage+=1
		elif _time_mined >= _time_to_mine*4/5 && anim_stage == 3:
			var source_id = map.get_cell_source_id(_mining_tile)
			map.set_cell(_mining_tile, source_id, map.ALMOST_MINED_COORDS)
		#head back when done
		if(_time_mined >= _time_to_mine):
			var source_id = map.get_cell_source_id(_mining_tile)
			map.set_cell(_mining_tile, source_id, map.MINED_COORDS)
			_current_tile = map.mine_base
			#could change later if we want miners to take multiple trips
			#carrying resources
			_mining_tile = Vector2i(0,0)
			anim_stage = 0
